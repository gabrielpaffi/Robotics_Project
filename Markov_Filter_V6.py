...
P_HIT = 0.8
# Probability that the robot moved to one of the 8 neighbouring "pixel"
P_MISS = 0.2/8 #8 neighbouring pixels
...
def initialize_maps(height,length):
    ...
    return map, map_estimate_robot, map_estimate_CV


def estimate_robot(x,y,map_estimate_robot): 
    ...
    return map_estimate_robot



def estimate_CV(x,y,confidence_CV,map_estimate_CV):
    # reset map to 0
    ...
    return map_estimate_CV
    

# Function to multiply probability maps and update the global map variable
def multiply_maps(map, map_estimate_CV, map_estimate_robot):
...
def filtered_pos_robot(map,map_estimate_robot):
